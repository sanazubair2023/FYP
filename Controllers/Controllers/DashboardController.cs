    ...
    public class DashboardController : Controller
    {
            ...
            [FromQuery] List<string>? subCategories = null)
        {
            ...
            foreach (var w in workerList)
            {
                ...
                // 🔥 UPDATED FALLBACK LOGIC: Check Bio first, otherwise fall back to "Servant / Maid"
                if (catNames.Any())
                {
                    workerSkillsMap[w.WorkerId] = catNames;
                }
                else if (!string.IsNullOrWhiteSpace(w.Bio))
                {
                    workerSkillsMap[w.WorkerId] = new List<string> { w.Bio };
                }
                else
                {
                    workerSkillsMap[w.WorkerId] = new List<string> { "Servant / Maid" };
                }
            }

            // Interview Status Map for logged-in Client
